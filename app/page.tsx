...
const mockLogs: LogEntry[] = [
  ...
  },
  {
    ...
    details: {
      workflow: { id: "wf-12345", step: "data-processing", duration: 45000 },
      error: {
        code: "TIMEOUT_ERROR",
...
export default function LoggingConsole() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const newLog: LogEntry = {
        ...
        source: ["workflow-engine", "token-service", "storage-monitor", "system"][Math.floor(Math.random() * 4)],
      }
  ...
  return (
    <div className={cn("min-h-screen transition-colors duration-200", isDarkMode ? "dark bg-gray-900" : "bg-gray-50")}>
      <div className="flex h-screen">
        ...
        >
          ...
          <div className="p-4 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-lg font-semibold text-gray-900 dark:text-white">Logs Explorer</h1>
          ...
          <div className="p-4 space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">Log Level</label>
          ...
          <div className="p-4 mt-auto border-t border-gray-200 dark:border-gray-700">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-gray-900 dark:text-white">{filteredLogs.length}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">Errors:</span>
                <span className="font-mono text-red-600">{filteredLogs.filter((l) => l.level === "error").length}</span>
              </div>
              <div className="flex justify-between text-sm">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* Logs Display */}
          <div className={cn("flex-1 overflow-auto font-mono text-sm", isDarkMode ? "bg-gray-900" : "bg-gray-50")}>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-3 mb-1">
                          ...
                          {log.source && <span className="text-gray-400 text-xs">{log.source}</span>}
                        </div>
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700 rounded p-3 text-xs">
                          {log.details.tokens && (
                            <div className="space-y-1">
                              ...
                              <div>Input: {log.details.tokens.input.toLocaleString()}</div>
                              <div>Output: {log.details.tokens.output.toLocaleString()}</div>
                              <div>Total: {log.details.tokens.total.toLocaleString()}</div>
                            </div>
                          )}

                          {log.details.storage && (
                            <div className="space-y-1">
                              <div className="font-semibold text-gray-700 dark:text-gray-300">Storage Info:</div>
                              <div>Used: {log.details.storage.used}</div>
                              <div>Limit: {log.details.storage.limit}</div>
                              <div>Usage: {log.details.storage.percentage}%</div>
                            </div>
                          )}

                          {log.details.workflow && (
                            <div className="space-y-1">
                              <div className="font-semibold text-gray-700 dark:text-gray-300">Workflow Details:</div>
                              <div>ID: {log.details.workflow.id}</div>
                              <div>Step: {log.details.workflow.step}</div>
                              <div>Duration: {log.details.workflow.duration}ms</div>
                            </div>
                          )}

                          {log.details.error && (
                            <div className="space-y-1">
                              <div className="font-semibold text-red-600 dark:text-red-400">Error Details:</div>
                              <div>Code: {log.details.error.code}</div>
